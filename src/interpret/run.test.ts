...
describe('analyzeDay', () => {
  ...
  it('runs detectAllRadar + interpret + persist for an extracted day', async () => {
    const deps = {
      getDay: vi.fn(async () => ({ status: 'extracted' })),
      loadInputs: vi.fn(async () => ({
        items: [{ day: '2026-06-13', ord: 1, segment: 'leader', title: 'x', summary: 'y' }],
        mentions: [{ day: '2026-06-13', term: 'x', count: 1 }],
        sectorSignals: [{ day: '2026-06-13', sector: '科技', polarity: 'bull' }],
      })),
    ...
    expect(deps.detect).toHaveBeenCalledWith(expect.objectContaining({ target: '2026-06-13' }));
